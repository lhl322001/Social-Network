    ...
    <el-container>
      ...
      <el-aside style="width: 500px">
        ...
        <div v-if="displayRelation">
          <el-collapse accordion class="related-human">
            <div  v-for="(human,key) in relatedHuman" :key="key">
              <el-collapse-item>
                <template slot="title">
                  {{ "可能认识的人："+human.name}}
                </template>
                  <div v-if="human.commonFriend.length">{{ human.commonFriend.length +"个共同好友"}}</div>
                        ...
                        <div v-if="human.sameCompany">现工作在同一工作单位</div>
                        <div v-if="human.sameCity">现工作在同一个城市</div>
                        <div v-if="human.sameHometown">曾经居住在同一所城市</div>
                        <div v-if="human.sameUniv">曾经就读于同一所大学</div>
                        <div v-if="human.sameMiddleSchool">曾经就读于同一所中学</div>
                        <div v-if="human.samePrimarySchool">曾经就读于同一所小学</div>
              </el-collapse-item>    
            </div>
          </el-collapse>
        </div>
